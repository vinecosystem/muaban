...
(() => {
  ...
  function toast(msg, ms = 2400) {
    ...
    app.ui.toast.textContent = msg;
    show(app.ui.toast);
    setTimeout(() => hide(app.ui.toast), ms);
  }
  ...
  async function init() {
    ...
    [app.ui.searchInput].forEach((el) => el?.addEventListener("input", debounce(loadMarket, 300)));
  ...
  async function connectWallet() {
    if (!window.ethereum) {
      toast("Cài ví để kết nối (Metamask/OKX…)");
      return;
    }
  ...
  async function readProduct(pid) {
    ...
    try {
      return await app.contracts.muaban.getProduct(pid);
    } catch {
      return await app.contracts.muaban.products(pid);
    }
  }
  ...
  async function loadMarket() {
    ...
    for (const pid of ids) {
      ...
      const taxPct = Number(p.taxRateBps) / 100;
  ...
  function openBuyModal(data) {
    ...
    const taxRate = Number(data.taxRateBps) / 100;
  ...
  function recalcBuy() {
    ...
    const taxRate = Number(currentBuy.taxRateBps) / 100;
