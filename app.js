...
// Muaban
const MUABAN_ABI = [
  {"inputs":[],"name":"registered","outputs":[{"internalType":"bool","name":"","type":"bool"}],"stateMutability":"view","type":"function"},
...
async function connectWallet(){
  ...
  hide($("#btn-connect")); show($("#btn-disconnect")); show($("#balances"));

  const isReg=await muaban.registered(account);
  if (!isReg) show($("#btn-register"));
  else { hide($("#btn-register")); show($("#btn-create")); show($("#btn-buyer-orders")); show($("#btn-seller-orders")); }
...
async function buyProduct(form){
  ...
  if(allow.lt(vinTotal)){ const txa=await vinW.approve(CONFIG.MUABAN_ADDR,vinTotal); await txa.wait(); }
...
function renderOrders(){
  ...
  bList.innerHTML=""; sList.innerHTML="";
  ordersCache.forEach((o)=>{
    ...
    if(o.buyer.toLowerCase()===account.toLowerCase()){
      const btn=document.createElement("button"); btn.textContent="Xác nhận nhận";
      btn.onclick=()=>confirmReceipt(o.orderId); li.appendChild(btn);
      bList.appendChild(li);
    }
    if(o.seller.toLowerCase()===account.toLowerCase()){
      const btn=document.createElement("button"); btn.textContent="Hoàn tiền";
      btn.onclick=()=>refund(o.orderId); li.appendChild(btn);
...
async function confirmReceipt(id){
  const muabanW=muaban.connect(signer);
  const tx=await muabanW.confirmReceipt(id); await tx.wait();
  await loadOrders();
}
async function refund(id){
  const muabanW=muaban.connect(signer);
  const tx=await muabanW.refundIfExpired(id); await tx.wait();
  await loadOrders();
}
